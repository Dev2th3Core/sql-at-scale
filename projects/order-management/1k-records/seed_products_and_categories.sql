...
-- Now using:
--   - dbo.ProductCategories (CategoryId, CategoryName)
--   - dbo.Products with CategoryId FK

USE OrderManagement;
GO

------------------------------------------------------------
-- 0. Clear existing data for repeatability during learning
------------------------------------------------------------
-- First clear Products (child), then ProductCategories (parent)
DELETE FROM dbo.Products;
DELETE FROM dbo.ProductCategories;
GO

------------------------------------------------------------
-- 1. Seed ProductCategories from a fixed list
------------------------------------------------------------
...
------------------------------------------------------------
-- 2. Helper lookup tables (in-memory) for random generation
------------------------------------------------------------
DECLARE @Categories TABLE (Id INT IDENTITY(1,1), CategoryName NVARCHAR(50), CategoryId INT);
DECLARE @Types      TABLE (Id INT IDENTITY(1,1), TypeName NVARCHAR(100));

-- Load categories from dbo.ProductCategories so we get real CategoryIds
INSERT INTO @Categories (CategoryName, CategoryId)
SELECT CategoryName, CategoryId
FROM dbo.ProductCategories
ORDER BY CategoryId;

-- Product "types" (names) - these will become ProductName directly
...
BEGIN
    -- Pick a random category row
    DECLARE @CategoryRow INT = 1 + ABS(CHECKSUM(NEWID())) % @CategoryCount;
    ...
    FROM @Categories
    WHERE Id = @CategoryRow;

    -- Pick a random product "type" (this will be ProductName)
    DECLARE @TypeRow INT = 1 + ABS(CHECKSUM(NEWID())) % @TypeCount;
    ...
    -- ProductName is just the type, e.g. "Wireless Mouse"
    DECLARE @ProductName NVARCHAR(200) = @TypeName;
    ...
    DECLARE @CategoryCode NVARCHAR(3) =
        UPPER(LEFT(REPLACE(@CategoryName, N' ', N''), 3));
    ...
    INSERT INTO dbo.Products (CategoryId, ProductName, Sku, UnitPrice, IsActive)
    VALUES (@CategoryId, @ProductName, @Sku, @UnitPrice, @IsActive);
...
SELECT TOP 10
    ...
    c.CategoryName
FROM dbo.Products p
JOIN dbo.ProductCategories c ON p.CategoryId = c.CategoryId
